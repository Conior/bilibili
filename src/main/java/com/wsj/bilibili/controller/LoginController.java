...
public class LoginController {
    ...
    public String login(@RequestParam("username") String username, @RequestParam("password") String password, HttpSession session,Model model){
        if (!StringUtils.isEmpty(username) && !StringUtils.isEmpty(password)){
            ...
            Integer account = accountService.getAccount(new AccountDTO(null, userName, passWord, null, null, 0));
    ...
    public String signup(HttpServletRequest request, HttpSession session, Model model){
        ...
        Integer account = accountService.addAccount(new AccountDTO(null, username, password, email, phone, 0));
        if (null != account && 0 != account) {
            session.setAttribute("loginUser", username);
            return "redirect:/main";
        }
